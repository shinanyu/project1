...
<script>
// Model 에 담긴 camp_no 변수 할당 + contextPath 길어서 변수 할당 
var camp_no = ${camp_no};
var page = ${page};
...
$(document).ready(function() {
	...
	// 상세정보 로딩
	getCampDetail(camp_no);
	
});

// 캠핑장 상세정보
function getCampDetail(camp_no){
	$.ajax({
		type : "GET",
		url : contextPath+"/campDetail/"+camp_no,
		success : function(result){
			// 상세내용
			var content = "<span style='text-align:'center';'><button onClick='ctlist()'>목록</button></a>"
				content += "<button onclick='campSpotLike("+result.camp_no+")'>추천</button></span>"
// 				content += "<button id='wishListButton' onclick='campWishList("+userno+","+result.camp_no+")'>찜하기</button>"
				content += "<table><tr><td colspan='2' style='text-align: center; width: 100%;'><img src="+result.camp_image+"></td></tr>"
			    content += "<tr><th style='width: 10%'>이름</th><td>"+result.camp_name+"</td></tr>"
			    content += "<tr><th style='width: 10%'>소개</th><td>"+result.camp_intro+"</td></tr>"
			    content += "<tr><th style='width: 10%'>주소</th><td>"+result.camp_addr+"</td></tr>"
			    content += "<tr><th style='width: 10%'>연락처</th><td>"+result.camp_tel+"</td></tr>"
			    content += "<tr><th style='width: 10%'>홈페이지</th><td><a href='"+result.camp_hp+"' class='link'>"+result.camp_hp+"</a></td></tr>"
			    content += "<tr><th style='width: 10%'>예약</th><td><a href='"+result.camp_resve+"' class='link'>"+result.camp_resve+"</a></td></tr></table>"
	    	...
	    	var mapOptions = {
				// 위치 지정
	    	    center: new naver.maps.LatLng(result.camp_lat, result.camp_long),
	    	    zoom: 15
	    	};
	    	var map = new naver.maps.Map('map', mapOptions);
	    	// 지도에 마커찍기
			var marker = new naver.maps.Marker({
			    position: new naver.maps.LatLng(result.camp_lat, result.camp_long),
			    map: map
			});
	    	var regid = result.camp_regid;
...
function getWeather(regid, startIndex){
	...
	$.ajax({
		...
		success : function(wresult){
			...
			weather +="<tr><td style='height:150px;'><img src='"+imgsrc+"' style='height: 100px; width: auto;'></td><td style='font-size:60px; font-weight:bold;'>"+wresult.slist[startIndex].tmp+"°C</td></tr>"
...
// 추천 기능
function campSpotLike(camp_no){
	$.ajax({
		type: "GET",
		url: contextPath+"/campSpotLike/"+camp_no,
...
<!-- 로그인후 인증정보가 넘어왔다면 로그아웃, 마이페이지 버튼이 생성됨 -->
